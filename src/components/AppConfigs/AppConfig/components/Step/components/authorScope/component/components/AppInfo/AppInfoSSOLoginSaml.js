...
class AppInfoSSOLoginSaml extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      Object.prototype.toString.call(this.state.assertion_attributes) !==
      Object.prototype.toString.call(nextProps.appDetail)
    ) {
      this.setState({
        assertion_attributes: nextProps.appDetail?.config?.assertion_attributes,
      });
    }
  }
  onGetSaveValue() {
  ...
  switchModal(show, newAttr, oldAttr) {
    if (newAttr) {
      const assertion_attributes = Array.isArray(this.state.assertion_attributes)
        ? [...this.state.assertion_attributes]
        : [];
      if (oldAttr) {
