  ...
  config.setRegisterHookFn((hooks) => {
    const list = [
      hooks.graphMeta.registerHook({
        ...
        handler: async (args) => {
          args.graphMetaService = ServiceApi.queryGraphMeta;
        },
      }),
      hooks.saveGraphData.registerHook({
        ...
        handler: async (args) => {
          if (!args.saveGraphDataService) {
            args.saveGraphDataService = ServiceApi.saveGraphData;
          }
        },
      }),
      hooks.addNode.registerHook({
        ...
        handler: async (args) => {
          args.createNodeService = ServiceApi.addNode;
        },
      }),
      hooks.delNode.registerHook({
        name: 'get edge config from backend api',
        handler: async (args) => {
          args.deleteNodeService = ServiceApi.delNode;
        },
      }),
      hooks.addEdge.registerHook({
        name: 'get edge config from backend api',
        handler: async (args) => {
          args.createEdgeService = ServiceApi.addEdge;
          args.edgeConfig = {
            ...args.edgeConfig,
      ...
      hooks.delEdge.registerHook({
        name: 'get edge config from backend api',
        handler: async (args) => {
          args.deleteEdgeService = ServiceApi.delEdge;
        },
      }),
    ];
  ...
  app.executeCommandPipeline([
    /** 1. 从服务端获取数据 */
    {
      ...
      getCommandOption: async () => {
        return {
          args: {
            loadDataService: ServiceApi.loadGraphData,
          },
        };
      },
    ...
    /** 2. 执行布局算法 */
    {
      ...
      getCommandOption: async (ctx) => {
        ...
        return {
          ...
          // },
          args: {
            customLayout: async (graphData1: NsGraph.IGraphData) => {
