...
import { getActionList, deleteAction } from '@/pages/Link/Manage/service';
...
const EventList: React.FC = (props: any) => {
    ...
    },
    {
      ...
      render: (_, record: any) => [
        <Button
          ...
          type="link"
          onClick={() => {
            history.push(`/link/manage/actionEdit?actionId=${record.id} &linkId=` + linkId);
          }}
        >
          编辑
        </Button>,
  ...
  const onAdd = () => {
    history.push(`/link/manage/actionAdd?linkId=` + linkId);
  };
  return (
    <ProTable
      actionRef={actionRef}
