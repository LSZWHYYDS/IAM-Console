import { Button, Checkbox, Col, Collapse, Form, Input, Row, Select, Spin } from 'antd';
...
import { getSnsByType } from '@/pages/UserCenter/SyncGene/service';
...
const EditDingDing: React.FC<any> = (props: any) => {
  ...
  const isCompSelect = Form.useWatch('isCompSelect', formBasic);
  const setFormData = (formData: any = {}) => {
    formBasic.setFieldsValue({
      ...formData,
      isCompSelect: !!formData.sns_id,
  ...
  const loadData = () => {
    getSnsAll(() => {
      if (id) {
        ...
        formBasic.setFieldsValue({ isCompSelect: true });
  ...
  useEffect(() => {
    ...
    if (id) {
      // 代表编辑
      // IdpRs = useRequest(() => getIdpType(id, 'IDP'));
...
export default connect(mapStateToProps, mapDispatchToProps)(EditDingDing);
