...
const WindowsAppList: React.FC = () => {
  ...
  const handleSaveAppVersion = async (fields: any, actionType: string) => {
    ...
    if (actionType === 'save') {
      appFields = { ...fields, platform: 'WINDOWS', fileId: appVersionId };
    } else {
      appFields = fields;
    }
    try {
      ...
      render: (_, appVersion) => {
        return [
          <Popconfirm
            ...
            title={`您当前${appVersion.status ? '下架' : '上架'} }的是【${
              currentApp?.appName
            }】客户端，版本号是：${appVersion.version}。`}
            onConfirm={() => {
              handlePushVersion(appVersion.id, appVersion.status);
            }}
            okText="确定"
            cancelText="取消"
          >
