...
const AndroidAppList: React.FC = () => {
  ...
  const handleSaveAppVersion = async (fields: any, actionType: string) => {
    ...
    if (actionType === 'save') {
      appFields = { ...fields, platform: 'IOS', fileId: appVersionId };
    } else {
      appFields = fields;
    }
    try {
  ...
  return (
    <>
      <ProTable<AppListItemType, TableListPagination>
        ...
        toolBarRender={() => [
          <UploadApp platformName="IOS" onFinished={(id) => handleGetUploadedAppInfo(id)} />,
        ]}
        columns={columns}
      />
      <ConfirmUpload
        ...
        platformName="IOS"
        confirmModalVisible={confirmUploadModalVisible}
        onCancel={() => {
