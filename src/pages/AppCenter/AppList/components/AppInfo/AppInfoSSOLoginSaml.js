...
import { previewSAML } from './servers';
...
class AppInfoSSOLoginSaml extends Component {
  ...
  componentDidMount() {
    console.log(this.props.samlData);
  ...
  collapseChange = (key) => {
    if (Array.isArray(key)) {
      if (key.includes('assertionPreview')) {
        // 发送调用接口
        const result = this.props.fetchData();
        result.then((rs) => {
          delete rs?.auth_protocol;
          previewSAML(rs).then((res) => {
            if (res) this.setState({ loading: false, xmlResult: res });
          });
        });
      }
    } else {
      if (key === 'assertionPreview') {
        // 发送调用接口
      }
    }
  };
              ...
              >
                <a href="#">
                  {location.protocol +
                    '//' +
                    location.host +
                    `/iam/saml2/${this.props.id_client}/metadata`}
                </a>
              </FormItem>
            </Col>
          </Row>
        ...
        <Panel header="SAML 高级配置" key="samlHigh">
          <Row>
            <Col span={24}>
              ...
              >
                <Select allowClear={true} defaultValue={true} style={{ width: '300px' }}>
