    ...
    if (id) {
      getLink(id).then((res) => {
        ...
        formBasic.setFieldsValue({
          SCMIAddree: window.location?.origin + `/iam/scim/${query?.NumberId}`,
        });
        ...
        <Panel header="基础配置" key="base">
          <Row>
            ...
            {query?.NumberId ? (
              <>
                <Col span="12">
                  <FormItem colon={false} name={'SCMIAddree'} label={'SCIM服务端地址'}>
  ...
  const render = () => {
    return (
      <div id="content" className="content">
        <Form form={formBasic} {...formItemLayout} autoComplete="off">
        ...
        <Row>
          <Col span="24" style={{ textAlign: 'center' }}>
            <Space>
              <Button onClick={onCancel} style={{ marginRight: 30 }}>
                取消
              </Button>
