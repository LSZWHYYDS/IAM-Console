...
class UserTagTree extends Component {
  constructor(...args) {
    super(...args);
    this.state = {
      data: [],
  ...
  // 获取所有key
  getAllKey = (treeData) => {
    _.forEach(treeData, (forIs) => {
      allKey.push(forIs['key']);
      if (forIs.children?.length) {
        this.getAllKey(forIs.children);
      }
    });
  };
